...
/**
 * NLP Service for processing natural language input.
 * Uses rule-based NLP with Apache OpenNLP-compatible patterns.
 * In production, integrate with OpenNLP models or an LLM API.
 */
@Service
@Slf4j
public class NlpService {
    ...
    /**
     * Extract key entities from the message (order numbers, emails, etc.)
     */
    public Map<String, String> extractEntities(String message) {
        Map<String, String> entities = new HashMap<>();

        // Extract order number (e.g., ORD-12345 or #12345)
        ...
        if (orderMatcher.find()) {
            entities.put("ORDER_NUMBER", orderMatcher.group(1));
        }
